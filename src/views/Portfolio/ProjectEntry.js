...
const ProjectEntry = (props: ProjectEntryProps) => {
  const { images, title, type, description, techs, links } = props.project
  const { setTopPosition, triggered } = props
  return (
    <div ref={e => setTopPosition(e)} className="project-entry">
      <div
        className={`project-entry-container fade-bottom ${
          triggered ? '' : 'fade-bottom--init'
        }`}
      >
